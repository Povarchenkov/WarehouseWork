...
Функция ПолучитьОтвет(СоединениеИПарметры, АдресРесурса, Метод, Знач ТелоЗапроса = Неопределено, Знач ОбрабатыватьОтвет = Истина, ПоляДаты = "Дата") Экспорт 
	...
	СтруктураОтвета = ПрочитатьJSONИзСтроки(ОтветСтрока,, ПоляДаты);
...
Функция ПолучитьКартинкуПоСсылке(НоменклатураСсылка, СоединениеИПараметры) Экспорт	
	АдресРесурсаСПараметрами = СоединениеИПараметры.Параметры.АдресРесурса + "/image?id=" + СокрЛП(НоменклатураСсылка.УникальныйИдентификатор());
		
	Возврат ОбменДаннымиКлиентСервер.ПолучитьОтвет(СоединениеИПараметры, АдресРесурсаСПараметрами, "GET");
КонецФункции
