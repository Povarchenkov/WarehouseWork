...
Процедура ОбработатьШтрихкод(Штрихкод)
	...
	Если СтруктураОтвета.Свойство("type") Тогда
		...
		ЭтоНоменклатура = СтруктураОтвета.type = "Номенклатура";
		...
		Если ЭтоНоменклатура
			...
			ЗарегистрироватьВИстории(Номенклатура, Ячейка);
...
&НаКлиенте
Процедура ЗарегистрироватьВИстории(Номенклатура, Ячейка)
	
	АдресРесурсаСПараметрами = СоединениеИПараметры.Параметры.АдресРесурса + "/registerCellForGoods?"
		+ "goods=" + Номенклатура.УникальныйИдентификатор()
		+ "&cell=" + Ячейка.УникальныйИдентификатор()
		+ "&author=" + ТекущийПользователь.УникальныйИдентификатор();
