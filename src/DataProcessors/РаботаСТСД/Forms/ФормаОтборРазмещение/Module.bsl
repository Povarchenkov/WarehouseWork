...
Процедура СписокЯчеекВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Ячейка = ЯчейкиОтбора.НайтиПоИдентификатору(ВыбраннаяСтрока).Значение;
	ТекущийЭтап = "ТоварыОтборПодборТовара";
	СканированиеЯчейкиОтбора();
	УправлениеФормой();
КонецПроцедуры
...
Процедура СброситьНоменклатуру(Команда)
	...
	Ячейка = Неопределено;
	КоличествоРазмещено = 0;
	ТекущийЭтап = "ТоварыОтборНачало";
	
	ОтобратьДанныеВОтборе(Неопределено);
	УправлениеФормой();
КонецПроцедуры
...
&НаКлиенте
Процедура ОтобратьДанныеВОтборе(СтруктураОтбора, ПоляОтбора = "")
...
&НаСервере
Процедура НастройкиПриСоздании() Экспорт
	...
	Элементы.Количество.Заголовок = "Отобрать";
	Элементы.КоличествоРазмещено.Заголовок = "Отобрано";
	
КонецПроцедуры

&НаКлиенте
Процедура СканированиеЯчейкиОтбора()
	ОтобратьДанныеВОтборе(Новый Структура("Ячейка", Ячейка), "Ячейка");			
КонецПроцедуры
