...
Процедура ПолучитьДокументПересчетТоваров(Команда)
	Если ЗначениеЗаполнено(Документ) Тогда
		ПараметрыФормы = Новый Структура("Ключ, ТекущаяЯчейка", Документ, Ячейка);
		Если КомпонентыДляСчитыванияШтрихкодов <> Неопределено Тогда
			КомпонентыДляСчитыванияШтрихкодов.Отключить(НомерУстройства);
	...
	ПоляДаты.Добавить("Дата");
	ПоляДаты.Добавить("ГоденДо");

	СтруктураОтвета = ОбменДаннымиКлиентСервер.ПолучитьОтвет(СоединениеИПараметры, АдресРесурсаСПараметрами, "GET",,, ПоляДаты);
