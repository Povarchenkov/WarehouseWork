...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТекущийУзел = ПланыОбмена.HTTPОбменСРабочейБазой.ЭтотУзел();
	
	Элементы.УстановкаЯчеекаДляТовара.Видимость = ТекущийПользователь.ДопВозможности;
	Элементы.ПересчетЯчейки.Видимость = ТекущийПользователь.ДопВозможности;
КонецПроцедуры
...
&НаКлиенте
Процедура ПересчетЯчейки(Команда)
	ОткрытьФорму("Обработка.РаботаСТСД.Форма.ФормаВыбораЯчейкиПересчета", Новый Структура("Пользователь", ТекущийПользователь), ЭтаФорма);	
КонецПроцедуры

&НаКлиенте
Процедура Сортировка(Команда)
	ОткрытьФорму("Обработка.РаботаСТСД.Форма.ФормаСортировки", , ЭтаФорма);	
КонецПроцедуры
...
Процедура ПеремещениеПоДокументу(Команда)
	Если ЗначениеЗаполнено(Документ) Тогда	
		...
		ОткрытьФорму("Обработка.РаботаСТСД.Форма.ФормаОтборРазмещение", ПараметрыФормы, ЭтаФорма);
	Иначе 
		Если НЕ ЗначениеЗаполнено(ТекущийПользователь) Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = "Не определен пользователь!";
